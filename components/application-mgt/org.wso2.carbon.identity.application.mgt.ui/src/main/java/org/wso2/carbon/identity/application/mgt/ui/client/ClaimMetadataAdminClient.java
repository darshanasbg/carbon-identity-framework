...
public class ClaimMetadataAdminClient {
    ...
    public Map<String, String[]> getClaimDialects() throws RemoteException,
            ClaimMetadataManagementServiceClaimMetadataException {

        Map<String, String[]> claimDialects = new HashMap<>();

        try {
            ClaimDialectDTO[] claimDialectDTOs = stub.getClaimDialects();

            if (claimDialectDTOs != null) {

                int i = 0;
                for (ClaimDialectDTO claimDialectDTO : claimDialectDTOs) {

                    String dialectURI = claimDialectDTO.getClaimDialectURI();

                    if (!"http://wso2.org/claims".equalsIgnoreCase(dialectURI)) {

                        ExternalClaimDTO[] externalClaimDTOs = getExternalClaims(dialectURI);
                        String[] externalClaimURIs;

                        if (externalClaimDTOs != null) {

                            externalClaimURIs = new String[externalClaimDTOs.length];

                            int j = 0;
                            for (ExternalClaimDTO externalClaimDTO : externalClaimDTOs) {
                                externalClaimURIs[j] = externalClaimDTO.getExternalClaimURI();
                                j++;
                            }

                        } else {

                            externalClaimURIs = new String[0];
                        }

                        claimDialects.put(claimDialectDTO.getClaimDialectURI(), externalClaimURIs);
                        i++;
